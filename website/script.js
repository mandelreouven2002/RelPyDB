...
const RELPY_FILES = ["__init__.py","hashing.py","exceptions.py","schema_types.py","schema.py","crud.py","ddl.py","export.py","indexes.py","queries.py","grouping.py","joins.py","persistence.py","encryption.py","tables.py"];
...
function initTOC() {
  ...
  if (!content) return;
  const pageTitle = content.querySelector('.docs-page-title');
  if (!pageTitle) return;
  ...
  toc.innerHTML = `<summary>On this page</summary><ol>${items}</ol>`;
