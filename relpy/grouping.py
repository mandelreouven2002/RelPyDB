...
class GroupedQuery:
    ...
    def _execute_rows(self) -> list[dict[str, Any]]:
        ...
        grouped_rows = self._core_grouped()
    ...
    def _core_join_grouped(self) -> list[dict[str, Any]] | None:
        """Fuses a lazy native inner join with this GROUP BY into one C pass,
        or returns None to fall back."""
        sq = self.source_query
        spec = getattr(sq, "core_join_spec", None)
        if spec is None:
            return None
        # only when the join is applied directly (no extra ops on the joined relation)
        if (sq.conditions or sq.selected_columns is not None
                or sq.order_specs or sq.limit_count is not None
                or sq.offset_count != 0 or sq.distinct_enabled):
            return None

        db = sq.db
        # reverse map: output prefixed name -> (side, source column)
        rev = {}
        for src, out in spec["left_map"].items():
            rev[out] = (0, src)
        for src, out in spec["right_map"].items():
            rev[out] = (1, src)

        def col_type(table, column):
            td = db.schema.get(table)
            if td is None or column not in td.columns:
                return None
            return td.columns[column].storage_type

        left_t, right_t = spec["left_table"], spec["right_table"]
        ...
        for gcol in self.group_columns:
            if gcol not in rev:
                return None
            side, src = rev[gcol]
            group_specs.append((side, src, gcol))

        numeric = {int, float}
        agg_specs = []
        for alias, aspec in self.aggregation_specs:
            func, column = aspec.function_name, aspec.column_name
            if func not in ("count", "sum", "avg", "min", "max"):
                return None
            if column is None:
                if func != "count":
                    return None
                agg_specs.append((alias, "count", 0, None))
                continue
            if column not in rev:
                return None
            side, src = rev[column]
            table = right_t if side else left_t
            if func in ("sum", "avg", "min", "max") and col_type(table, src) not in numeric:
                return None
            agg_specs.append((alias, func, side, src))

        engine = db._core_join_engine(left_t, right_t)
        if engine is None:
            return None

        return engine.hash_join_group(
            left_t, spec["left_plan"], spec["left_keys"],
            right_t, spec["right_plan"], spec["right_keys"],
            group_specs, agg_specs,
        )

    def _core_grouped(self) -> list[dict[str, Any]] | None:
        """
        Executes GROUP BY + aggregates in the native C engine when the source is
        a plain single scalar table with a fully structured WHERE and only
        count/sum/avg/min/max aggregates. Returns the grouped rows, or None to
        fall back to the Python path. HAVING / ORDER BY / SELECT / LIMIT are
        applied afterwards by the normal pipeline, so this only accelerates the
        heavy grouping step.
        """
        from .queries import Condition, OP_AND

        sq = self.source_query

        # Fused inner-join + GROUP BY: if the source is a lazy native inner join
        # applied directly (no conditions/select/order/limit of its own), do the
        # join and the grouping together in one C pass — no intermediate rows.
        fused = self._core_join_grouped()
        if fused is not None:
            return fused

        if (getattr(sq, "table_name", None) is None
                or getattr(sq, "source_rows", None) is not None
                or getattr(sq, "core_join_spec", None) is not None
                or sq.selected_columns is not None
                or sq.limit_count is not None
                or sq.offset_count != 0
                or sq.distinct_enabled):
            return None

        mirror = sq.db._core_mirror_for(sq.table_name)
        if mirror is None:
            return None

        # structured WHERE plan (or None)
        plan = None
        for condition in sq.conditions:
        ...
        table_columns = sq.db.schema[sq.table_name].columns
        for gc in self.group_columns:
            if gc not in table_columns:
                return None
        ...
        for alias, spec in self.aggregation_specs:
            ...
            if func in ("sum", "avg", "min", "max"):
                if not sq.db._core_numeric_column(sq.table_name, column):
                    return None
            if func == "count" and column is not None and column not in table_columns:
                return None
            aggs.append((alias, func, column))

        engine, _cols = mirror
        return engine.group(sq.table_name, plan, list(self.group_columns), aggs)
    ...
    def _build_grouped_rows(
        ...
        # Only build the decrypted query view per row when the schema actually
        # has encrypted columns; otherwise the stored row is already usable.
        needs_view = self.source_query.db._query_view_needs_processing()

        for row in rows:
            query_row = (
                self.source_query.db._row_for_query_execution(row)
                if needs_view else row
            )

            group_key = tuple(
                make_hashable(query_row[column_name])
    ...
    def _json_default(self, value: Any) -> Any:
        ...
        Uses source_query.db._json_default if it exists.
