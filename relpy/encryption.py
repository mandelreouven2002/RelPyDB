...
from ._bootstrap import ensure as _ensure
_fernet = _ensure("cryptography.fernet", "cryptography")
...
class EncryptionMixin:
    ...
    def _query_view_needs_processing(self) -> bool:
        """
        Whether query rows need per-row processing before predicates run.

        This is True only if the schema declares at least one encrypted column
        anywhere. If no column is encrypted, no encrypted payload can exist, so
        the stored row is already a valid query view and the expensive per-row
        scan/transform in :meth:`_row_for_query_execution` can be skipped
        entirely. Computed once per query (O(number of columns)), never per row.
        """

        for table_def in self.schema.values():
