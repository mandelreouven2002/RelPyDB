...
def join_queries(
    ...
    *,
    ...
    # --- native C inner-join fast path (lazy) --------------------------
    if join_type == "inner":
        spec = _build_core_join_spec(
            left_query, right_query, join_pairs,
            left_columns, right_columns, left_name, right_name,
            left_prefix, right_prefix, prefix_columns,
            preserve_prefixed_columns, suffixes, match_nulls,
        )
        if spec is not None:
            joined_spec, output_columns = spec
            relation_name = f"{left_name}_inner_join_{right_name}"
            return Query(
                db=left_query.db,
                relation_name=relation_name,
                source_columns=tuple(output_columns),
                core_join_spec=joined_spec,
            )
...
def _normalize_join_type(method: str) -> str:
    ...
    if normalized not in JOIN_METHOD_ALIASES:
        raise QueryError(
            ...
            "Supported types: regular, inner, left, right, full, cross, natural."
        )
...
def _core_query_simple(q) -> bool:
    return (getattr(q, "table_name", None) is not None
            and getattr(q, "source_rows", None) is None
            and q.selected_columns is None
            and q.limit_count is None
            and q.offset_count == 0
            and not q.distinct_enabled
            and not q.order_specs)


def _core_structured_plan(q):
    """Returns (ok, plan): ok=False if any condition can't be expressed as a
    structured plan; plan may be None when there is no WHERE."""
...
def _build_core_join_spec(left_query, right_query, join_pairs,
                          left_columns, right_columns, left_name, right_name,
                          left_prefix, right_prefix, prefix_columns,
                          preserve_prefixed_columns, suffixes, match_nulls):
    """Builds a native inner-join spec (for lazy C execution), returning
    (spec, output_columns), or None to fall back to the Python join."""
    if match_nulls or not join_pairs:
        return None
    db = left_query.db
    if getattr(right_query, "db", None) is not db:
        return None
    if not (_core_query_simple(left_query) and _core_query_simple(right_query)):
        return None
    if db._core_join_engine(left_query.table_name, right_query.table_name) is None:
        return None

    ok_left, left_plan = _core_structured_plan(left_query)
    ok_right, right_plan = _core_structured_plan(right_query)
    ...
    spec = {
        "left_table": left_query.table_name,
        "left_plan": left_plan,
        "left_keys": [pair[0] for pair in join_pairs],
        "right_table": right_query.table_name,
        "right_plan": right_plan,
        "right_keys": [pair[1] for pair in join_pairs],
        "left_map": left_column_map,
        "right_map": right_column_map,
        "output_columns": list(output_columns),
    }
    return spec, output_columns
